   ...
   public function testCacheWithEmptyRoutesDirectory(): void
   {
       ...
       mkdir($emptyDir, 0777, true);
        ...
        try {
            ...
            // Vérifier la structure du cache même si vide
            $this->assertArrayHasKey('files', $routeCache);
            $this->assertIsArray($routeCache['files']);
            $this->assertEmpty($routeCache['files']);
            
            // Obtenir les informations du cache pour vérifier les compteurs
            $cacheInfo = $cache->getCacheInfo();
            $this->assertArrayHasKey('routes_count', $cacheInfo);
            $this->assertArrayHasKey('dynamic_dirs', $cacheInfo['routes_count']);
            $this->assertArrayHasKey('files', $cacheInfo['routes_count']);
            
            $this->assertEquals(0, $cacheInfo['routes_count']['dynamic_dirs']);
            $this->assertEquals(0, $cacheInfo['routes_count']['files']);
        } finally {
            // Nettoyer le répertoire temporaire même en cas d'exception
            if (is_dir($emptyDir)) {
                rmdir($emptyDir);
            }
        }
...
//
    ...
    public function testCacheWithNonExistentRoutesDirectory(): void
    {
        ...
        // S'assurer que le répertoire n'existe vraiment pas
        ...
        try {
            ...
            // Restaurer le niveau d'erreur précédent
